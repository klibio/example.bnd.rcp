...
class LauncherTracker extends ServiceTracker {
	...
	// this is more of an Eclipse argument 
	// but this OSGi implementation stores
	// its metadata alongside Eclipse's.
	private static final String DATA = "-data"; //$NON-NLS-1$

	// eclipse system properties
	public static final String            PROP_BUNDLES = "osgi.bundles"; //$NON-NLS-1$
	// start-level used to install bundles
	...
	// shape of the framework (jar, or folder)
	...
	public Object addingService(ServiceReference reference) {
		...
		return context.registerService(ApplicationLauncher.class.getName(), new EclipseApplicationLauncher(context),
				null);
	}
	...
	private void prepareEnvService(BundleContext context) {
		...
		ServiceReference<EnvironmentInfo> srvcRef = bc.getServiceReference(EnvironmentInfo.class);
		if (srvcRef != null) {
		...
		try {
			...
			if (serviceReferences != null && serviceReferences.length > 0) {
				...
				if (property != null && property instanceof String[] ) {
					String[] parameters = (String[]) property;
	...
	private static void processCommandLine(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i - 1].equalsIgnoreCase(APPLICATION)) {
				setProperty(PROP_APPLICATION, arg);
				found = true;
			}
